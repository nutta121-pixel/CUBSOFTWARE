...
//  WHAT THIS DOES
//  !so @hexechotv
//    1. Looks up the last game they played (works even offline)
//    2. AI writes something funny about that game
//    3. Sends it in chat with a link to their channel
// ============================================================
//
//  QUICK START - add these 3 global vars in Streamer.bot
//  Settings -> Global Variables
//
//    groqApiKey         Free AI key  -  sign up at https://console.groq.com
//    twitchClientId     From https://dev.twitch.tv/console/apps
//    twitchClientSecret From the same app page
//
//    Register any app name, set redirect URL to http://localhost
//    The code handles the Twitch token automatically - no manual steps.
//
// ============================================================
//  CUSTOMISATION - optional global vars
// ============================================================
//
...
//    Default    : hype
//
//    hype         - Big energy. Caps. Maximum excitement.
//    funny        - Pun or joke about the game they play.
//    chill        - Relaxed, like a friend recommending someone.
//    chaotic      - Unhinged meme energy. Unpredictable.
//    professional - Clean and formal. No slang.
//
//  EXTRA INSTRUCTIONS - bolt extra rules onto any style
//    Global var : shoutoutExtra
//    Examples   : "Always mention their follower count."
//                 "Always end with a relevant emoji."
//                 "Always include a fun fact about the game."
//
//  AI MODEL - swap the Groq model
//    Global var : shoutoutModel
//    Default    : llama-3.1-8b-instant  (fast, free)
//    Smarter    : llama-3.3-70b-versatile  (better output, still free)
//
//  CUSTOM PROMPT - override the whole AI instruction
//    Global var : shoutoutPrompt
//    Write your own instruction. Use {tokens} for live data.
//
//    Tokens available:
...
//      {age}        How long on Twitch  (e.g. "3 years")
//      {since}      Month they joined   (e.g. "January 2021")
//      {by}         Who triggered the !so command
//      {today}      Today's date
//
//    Example prompts:
//      "Roast {name} for playing {game} then send chat to {link}."
//      "Write a movie trailer voice-over for {name}'s {game} stream. Include {link}."
//      "Give {name} a shoutout and include a fun fact about {game}. End with {link}."
//
//  DISCORD LOGGING (optional - same vars used across all CUB commands)
...
public class CPHInline
{
    public bool Execute()
    {
        try
        {
            ...
            LogCommand("!so", by, "Target: " + target);
            ...
            // -- Fetch last played game via Twitch API (works offline) -----------------
            string game  = "";
            string title = "";
            string tags  = "";
            ...
            // Fall back to sub-action args if Twitch API creds are not set
            ...
            // -- Generate and send the shoutout ----------------------------------------
            string msg = GenerateShoutout(name, login, game, title, bio, link,
                                          followers, tags, badge, age, since, status, by, today);

            // ==========================================================================
            // TWITCH CHAT MESSAGE
            // The AI-generated message is sent when Groq returns a result.
            // The fallback line below is sent if Groq fails or groqApiKey is not set.
            // To change what the AI writes: edit the DEFAULT PROMPT section below.
            // To change the fallback: edit the string on the next line.
            // ==========================================================================
            string fallback = "Go check out " + name + " at " + link + " -- they're great!";
            // EDIT THE LINE ABOVE to change the fallback message.
            // You can use any of these variables in it: name, login, game, link, badge, status
    ...
    // ==========================================================================
    // TWITCH CHANNEL INFO
    // Calls /helix/channels which returns the last game even when offline.
    // The App Access Token is fetched and cached automatically.
    // Returns string[3]: { game_name, title, tags }
    // Returns null if twitchClientId / twitchClientSecret are not set.
    // ==========================================================================
    private string[] FetchChannelInfo(string login)
    {
        try
        {
            ...
            {
                CPH.LogInfo("[AI Shoutout] twitchClientId / twitchClientSecret not set. Game lookup disabled.");
                return null;
            }
            ...
            // Step 1 - resolve login to broadcaster_id
            string usersResp;
            ...
            // Step 2 - get channel info by broadcaster_id
            string chanResp;
            using (var wc = new WebClient())
            {
                wc.Headers.Add("Client-Id", clientId);
            ...
            return new string[] { game, title, tags };
        }
        catch (Exception ex) { CPH.LogError("[AI Shoutout] FetchChannelInfo: " + ex.Message); return null; }
    }

    // Fetches a Twitch App Access Token and caches it. Auto-refreshes when expired.
    ...
    // ==========================================================================
    // AI SHOUTOUT GENERATION  (Groq - free at console.groq.com)
    // ==========================================================================
        ...
        string badge, string age,   string since, string status, string by, string today)
    {
        try
        {
            ...
            // Check for a custom prompt template first
            string custom = CPH.GetGlobalVar<string>("shoutoutPrompt", true);
            string userPrompt;

            if (!string.IsNullOrEmpty(custom))
            {
                // Replace {tokens} with live values and use as the prompt
                userPrompt = custom
                    .Replace("{name}",      name)
                    .Replace("{login}",     login)
                    .Replace("{game}",      game)
            ...
            {
                // ==================================================================
                // DEFAULT PROMPT - edit this to change what the AI writes
                //
                // This runs when shoutoutPrompt global var is NOT set.
                // The text here is the instruction given to the AI.
                // The AI then writes the actual chat message based on it.
                //
                // Variables you can use anywhere in this prompt:
                //   name    = display name      game   = last played game
                //   link    = twitch.tv/login   title  = stream title
                //   bio     = channel bio       tags   = channel tags
                //   badge   = Partner/etc       status = LIVE or offline
            ...
            if (!m.Success) { CPH.LogWarn("[AI Shoutout] Could not parse Groq response: " + respBody); return null; }
    ...
    // ==========================================================================
    // STYLE PRESETS
    // Set the shoutoutStyle global var to one of these values:
    //   hype         - Big energy, caps, max excitement         (default)
    //   funny        - Pun or joke about the game
    //   chill        - Relaxed, genuine friend recommendation
    //   chaotic      - Unhinged meme energy, gremlin vibes
    //   professional - Clean and formal, no slang
    // ==========================================================================
    private string GetStyle(string style)
    {
        ...
        {
            case "funny":        return "Be funny. Include a pun or joke specifically about the game they play.";
    ...
    // DISCORD LOGGING
    // ==========================================================================
    private const int C_OK   = 5763719;
    private const int C_WARN = 16705372;
    private const int C_ERR  = 15548997;
    private const int C_CMD  = 10181046;
