"""
CUB SOFTWARE Error Reporter (Python)

Provides structured error reporting with custom error codes, console logging,
and Discord incident channel posting for Python CUB SOFTWARE applications.

Usage:
    from cub_error_reporter import report_error, ERRORS

    # Set bot token once (at app startup):
    import cub_error_reporter
    cub_error_reporter.BOT_TOKEN = os.environ.get('CUB_PROTECTOR_TOKEN', '')

    # Report an error:
    report_error(ERRORS['WEBSITE']['API_HANDLER'], 'Unhandled /api/foo exception',
                 ctx={'route': '/api/foo', 'user': 'test'}, err=e, app_name='Website')
"""
...
INCIDENT_CHANNEL_ID = '1493043555931783168'

# Set this at application startup with the CUB PROTECTOR bot token
# (or any bot token that has access to the incident channel)
BOT_TOKEN = os.environ.get('CUB_PROTECTOR_TOKEN', '')

# ─── Error Code Catalog  (globally unique 001–168) ────────────────────────────
ERRORS = {
    ...
    # ── CLEANME — core (018–023) + additional (151–157) ──────────────────────
    'CLEANME': {
    ...
    # ── WEBSITE — core (024–027) + additional (158–168) ──────────────────────
    'WEBSITE': {
...
ERROR_CATALOG = [
    ...
    {'code': ERRORS['CUBPROTECTOR']['CUBAI_WHISPER_CRASH'],   'app': 'CUB PROTECTOR', 'severity': 'fatal',   'desc': 'Whisper process crashed — Python deps missing'},
...
def _post_to_discord_async(code, message, ctx, err, app_name, severity):
    """Posts an embed to the incident channel using the Discord REST API (non-blocking)."""
    token = BOT_TOKEN
    if not token:
        return

    icon = _SEVERITY_ICONS.get(severity, '❌')
    color = _SEVERITY_COLORS.get(severity, 0xef4444)

    fields = []
    for k, v in (ctx or {}).items():
        if v is not None:
            fields.append({'name': str(k), 'value': str(v)[:1024], 'inline': True})
    ...
    embed = {
        'title': f'{icon} {severity.upper()} — {app_name}',
        'description': f'```{code}```\n{message}',
        'color': color,
        'fields': fields,
    ...
    url = f'https://discord.com/api/v10/channels/{INCIDENT_CHANNEL_ID}/messages'
    ...
    try:
        req = urllib.request.Request(
            ...
            headers={
                ...
                'User-Agent': 'CUBSOFTWAREErrorReporter/1.0',
            },
            method='POST',
        )
        ...
        print(f'[ErrorReporter] Failed to post to incident channel: {post_err}', file=sys.stderr)


def report_error(code, message, ctx=None, err=None, app_name='CUB SOFTWARE'):
    """
    Report an error: log to stdout/stderr + post embed to Discord incident channel.

    :param code:     Error code string from ERRORS catalog
    :param message:  Human-readable summary
    :param ctx:      Dict of additional context fields
    :param err:      Original exception (for stack trace)
    :param app_name: App name shown in the embed title
    """
    entry = next((e for e in ERROR_CATALOG if e['code'] == code), None)
    severity = entry['severity'] if entry else 'error'
    icon = _SEVERITY_ICONS.get(severity, '❌')

    pm2_line = f'[Error] {code} — {message}'
    if severity in ('fatal', 'error'):
        print(pm2_line, file=sys.stderr)
        if err:
            traceback.print_exc()
    else:
        print(pm2_line)

    # Post to Discord in a background thread so it never blocks a request
    t = threading.Thread(
        target=_post_to_discord_async,
        args=(code, message, ctx, err, app_name, severity),
        daemon=True,
    )
    t.start()
